...
- [x] オートメーション一覧が表示される（空状態・正常）← 2026-04-02 Playwright確認
- [ ] 新規作成モーダルが開く
- [ ] トリガー・条件・アクションを設定して保存できる
- [ ] 有効/無効の切り替えが動作する

#### 9-3: スコアリング `/scoring`
- [x] スコアリングルール一覧が表示される（空状態・正常）← 2026-04-02 Playwright確認
- [ ] 新規ルールを作成できる（イベント・ポイント設定）
- [ ] 友だち一覧でスコアが反映されている

#### 9-4: Webhook `/webhooks`
- [x] 受信 Webhook 一覧が表示される（空状態・正常）← 2026-04-02 Playwright確認
- [x] 送信 Webhook 一覧が表示される（空状態・正常）← 2026-04-02 Playwright確認
- [ ] 受信 Webhook を新規作成できる
- [ ] 送信 Webhook を新規作成できる

#### 9-5: 通知 `/notifications`
- [x] 通知ルール一覧が表示される（空状態・正常）← 2026-04-02 Playwright確認
- [ ] 新規通知ルールを作成できる（トリガー・通知先設定）

#### 9-6: フォーム回答 `/form-submissions`
- [x] フォーム一覧が表示される（空状態・正常）← 2026-04-02 Playwright確認
- [ ] フォームを選択すると回答一覧が表示される

#### 9-7: CV計測 `/conversions`
- [x] コンバージョンポイント一覧が表示される（空状態・正常）← 2026-04-02 Playwright確認
- [ ] 新規 CV ポイントを作成できる
- [ ] CV イベント履歴が表示される

#### 9-8: 流入経路 `/affiliates`
- [x] 流入元（ref コード）ごとのサマリーが表示される（空状態・正常）← 2026-04-02 Playwright確認
- [ ] 詳細ドリルダウンが動作する

#### 9-9: BAN検知 `/health`
- [x] アカウントヘルス情報が表示される（LINEアカウント未登録のため空）← 2026-04-02 Playwright確認
- [ ] ヘルスチェック実行ボタンが動作する

#### 9-10: UUID管理 `/users`
- [x] UUID ユーザー一覧が表示される（1件表示・正常）← 2026-04-02 Playwright確認
- [ ] ユーザー詳細（紐付き友だち）が確認できる

#### 9-11: LINEアカウント `/accounts`
- [x] LINE アカウント一覧が表示される（空状態・正常）← 2026-04-02 Playwright確認
- [ ] アカウント詳細情報が正しく表示される

#### 9-12: スタッフ管理 `/staff`
- [x] スタッフ一覧が表示される（空状態・正常）← 2026-04-02 Playwright確認
- [ ] 新規スタッフを招待・作成できる

#### 9-13: 緊急コントロール `/emergency`
- [x] 緊急停止・全配信停止ボタンが表示される ← 2026-04-02 Playwright確認
- [ ] 誤操作防止の確認ダイアログが出る

---

## 📍 次回セッション引き継ぎ（最終更新: 2026-04-02）
- 現在取り組んでいる箇所: **Phase 9 動作確認ツアー進行中**
- **次にやること:** 各ページの「作成・保存」インタラクション確認（9-2〜9-13 の残チェック）
- Playwright セットアップ完了: `apps/e2e/` — `pnpm exec playwright test` で実行
